...
public class Spell : MonoBehaviour
{
    ...
    [Header("Script to handle any additional effect you want to have")]
    //Take in the bonus effect script
    [SerializeReference] public List<SecondaryStructure> bonus = new List<SecondaryStructure>(); //make an array of bonus effects in case a spell has multiple?

    //public List<MonoBehaviour> behaviours;
    // public IEnumerable<SecondaryStructure> bonus;

    //private SphereCollider spellSphere;
    private Collider spellSphere; 
    private Rigidbody spellRigid;

    //As we take the sphere collider as the default code assumes a cube collider has been attached and configured to desire size
...
/*        if(spellToCast.spellAttackType == SpellScriptableObject.attackTypes.cube)
        {
            this.gameObject.AddComponent<BoxCollider>();
            box = this.gameObject.GetComponent<BoxCollider>();
            box.isTrigger = true;
            //box.size = spellToCast.spellRadius;
        }*/
    ...
    private void Start()
    {
/*        foreach(SecondaryStructure bonusEffect in bonus)
        {
            bonusEffect.caster = caster;
            bonusEffect.applyBonusEffect();
        }*/
    }
